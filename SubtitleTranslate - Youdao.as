...
/** 获取当前插件的标题 */
string GetTitle(){
    return "{$CP950=有道翻译$}{$CP936=有道翻译$}{$CP0=You Dao translate$}";
}


/** 获取当前插件的表述信息 */
string GetDesc(){
    return "https://ai.youdao.com/DOCSIRMA/html/trans/api/wbfy/index.html";
}
...
string Translate(string text, string &in srcLang, string &in dstLang){
    ...
    if(!text.empty()){//确实有内容需要翻译才有必要继续
        ...
        if(!html.empty()){//如果成功取得 Html 内容
          ret = JsonParseNew(html);//那么解析这个 HTML 里面的 json 内容
        }
